...
## 如何透过汇入 JSON 档案建立产品原型

除了自行建立产品原型外，您亦可选择透过汇入来建立一个其他使用者分享的产品原型。您同时也可以汇出您的产品原型并分享给其他使用者。
...
请注意，在汇出的 JSON 档案中，我们会以 http://localhost 取代您在**触发条件与动作**中设定的Webohook URL，以防止他人在直接汇入此 JSON 档案后，将其装置资讯上传至您的服务器。
